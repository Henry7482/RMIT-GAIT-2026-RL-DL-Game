...
### Observation Vector (18-dimensional)
Normalized feature vector including:
- Player state (8): x, y, vx, vy, velocity_magnitude, wall_proximity, angle, health
- Nearest enemy (3): distance, cos(angle), sin(angle)
- Nearest spawner (3): distance, cos(angle), sin(angle)
- Game state (4): phase, enemy_count, spawner_count, can_shoot
...
2. **Create streamlined README.md** in the checkpoint folder. Focus on behavior-influencing changes:

   **CRITICAL SECTIONS (for model recreation):**
   - **Observation Space Configuration** ⭐ MUST INCLUDE - Full 18-dimensional breakdown with exact normalization formulas
     - Document each dimension: what it represents, how it's normalized, value ranges
     - Include observation space Box configuration (low=-1.0, high=1.0, shape=(18,))
     - This is ESSENTIAL for loading and using the model correctly
   - **Key Changes from Previous Version** - Focus ONLY on params/hyperparams/rewards that influenced behavior
     - What changed (with exact values)
     - Why it changed (hypothesis/rationale)
     - How it affected behavior (observed impact)
   - **Complete Training Configuration** - All PPO hyperparameters in table format
   - **Complete Reward Function** - All rewards/penalties with values
   - **Action Space** - 5 discrete actions with exact game mechanics (thrust=0.3, rotation=5°/frame, etc.)
   ...
   **SUPPORTING SECTIONS:**
   ...
   - **Evolution Timeline** - Track how each parameter changed over time
   - **Goals for Next Version** - What to try next
   - **Performance Summary** - Metrics comparison with TensorBoard run ID
...
See existing checkpoints for examples:
- `models/non-suicidal-1.3_20260112_1645/README.md` - ⭐ BEST EXAMPLE - Streamlined, behavior-focused
- `models/non-suicidal-1.2_20260111_2202/README.md` - Corner escape breakthrough
- `models/non-suicidal-1.1_20260111_1612/README.md` - Optimal learning rate schedule

**Streamlined template focusing on behavior changes:**
```markdown
...
**TensorBoard Run:** `PPO_{run_number}`

## Status Summary
[1-2 sentences: What's the key breakthrough? What behavior improved?]

## Key Changes from v{Previous} (Behavior-Influencing)
...
**Why this matters:**
- [How this change affected the policy gradient]
- [What new behaviors it enabled]
- [Why agent learned differently]

### 2. [Secondary Change]
[Same structure...]

## Complete Training Configuration
[Table with ONLY params that changed, or full table if first version]

## Complete Reward Function
[Table highlighting changes from previous version]

## Observation Space Configuration (18-dimensional) ⭐ CRITICAL
[Full breakdown with normalization formulas - REQUIRED for model recreation]

## Action Space (Discrete - 5 actions)
[With exact game mechanics values]

## Why This Version Works: Behavior Analysis
[Explain the policy gradient/reward structure that drives the new behavior]

## Performance Metrics (PPO_{run_number})
[Table comparing to previous version]
...
4. **Key discoveries to document:**
   ...
   - Entropy coefficient: 0.065-0.1 range (0.065 for cautious, 0.1 for aggressive exploration)
   - Removing penalties > adding rewards: Setting potential_further=0.0 enabled thrust more than increasing potential_closer
   - Reward balance: destroy_enemy=170, destroy_spawner=180, take_damage=-80, death=-200
   - Observation space: 18-dimensional (MUST document exact normalization for model recreation)
